...
import os, sys, json, shutil, threading, logging, string, random
from pathlib import Path
from flask import Flask, jsonify, send_from_directory, abort, request, make_response
...
for i in range(1, 4):
    (SLOTS / f"option{i}").mkdir(parents=True, exist_ok=True)
    (RESTORE / f"restore{i}").mkdir(parents=True, exist_ok=True)
...
def init_files():
    ...
    if not KEYSF.exists():
        KEYSF.write_text(json.dumps({"keys": ["ZEX-MASTER-9999-ROOT"]}, indent=2), encoding="utf-8")
...
def scfg(c):
    CFGF.write_text(json.dumps(c, indent=2, ensure_ascii=False), encoding="utf-8")
...
def lkeys():
    try:
        return json.loads(KEYSF.read_text(encoding="utf-8"))
    except Exception:
        return {"keys": {}}

def skeys(k):
    KEYSF.write_text(json.dumps(k, indent=2), encoding="utf-8")

def resolve_slot_dir(opt, sid):
    opt_str = str(opt).strip().lower()
    if not opt_str.startswith("option"):
        opt_str = f"option{opt_str}"
    p = SLOTS / opt_str / str(sid)
    p.mkdir(parents=True, exist_ok=True)
    return p

def slot_dir(opt, sid):
    return resolve_slot_dir(opt, sid)

def file_url(opt, sid, fname):
    opt_num = str(opt).replace("option", "")
    return f"{VPS_URL}/slots/option{opt_num}/{sid}/{fname}"

def next_id(slots):
    return max((s["id"] for s in slots if isinstance(s, dict) and "id" in s), default=0) + 1

def slot_info_text(slot, opt):
    folder = resolve_slot_dir(opt, slot["id"])
...
def add_cors_headers(response):
    ...
    response.headers["Access-Control-Allow-Headers"] = "Content-Type, Authorization, X-File-Name, x-file-name"
    response.headers["Access-Control-Expose-Headers"] = "X-File-Name, x-file-name, Content-Disposition"
    return response

@web.route("/config")
def api_config():
    return web.response_class(
        response=CFGF.read_text(encoding="utf-8"),
        status=200, mimetype="application/json")

@web.route("/keys")
def api_keys():
    return web.response_class(
        response=KEYSF.read_text(encoding="utf-8"),
        status=200, mimetype="application/json")

@web.route("/health")
def health():
    return jsonify({"status": "ok", "url": VPS_URL, "port": PORT})

@web.route("/restore/<num>")
...
def serve_restore(num):
    ...
    try:
        ...
        if not folder.exists():
            return jsonify({"error": f"Restore folder {num} not found"}), 404
        files = [f for f in folder.iterdir() if f.is_file()]
        if not files:
            return jsonify({"error": f"No restore file uploaded in restore{num}"}), 404
        f = files[0]
        ...
        log.error(f"Error in serve_restore: {ex}", exc_info=True)
        return jsonify({"error": str(ex)}), 500

@web.route("/slot-file/<opt>/<sid>")
@web.route("/slot-file/<int:opt>/<int:sid>")
def auto_pickup(opt, sid):
    """Auto pickup — first file from slot folder, no URL needed!"""
    try:
        folder = resolve_slot_dir(opt, sid)
        if not folder.exists():
            return jsonify({"error": f"Slot folder option{opt}/{sid} not found"}), 404
        # Get binary files, ignoring cover photos and hidden files
        files = [f for f in folder.iterdir() if f.is_file() and not f.name.startswith("cover") and not f.name.startswith(".")]
        if not files:
            files = [f for f in folder.iterdir() if f.is_file()]
        if not files:
            return jsonify({"error": f"No file uploaded in slot {sid}"}), 404
        pick = files[0]
        ...
        log.error(f"Error in auto_pickup: {ex}", exc_info=True)
        return jsonify({"error": str(ex)}), 500
...
def serve_file(opt, sid, filename):
    ...
    try:
        folder = resolve_slot_dir(opt, sid)
        if not folder.exists():
            return jsonify({"error": f"Slot directory not found"}), 404
        file_path = folder / filename
        if not file_path.exists():
            return jsonify({"error": f"File {filename} not found"}), 404
        resp = send_from_directory(str(folder), filename, mimetype="application/octet-stream")
        resp.headers["X-File-Name"] = filename
        resp.headers["x-file-name"] = filename
        return resp
    except Exception as ex:
        log.error(f"Error in serve_file: {ex}", exc_info=True)
        return jsonify({"error": str(ex)}), 500
...
def kb_main():
    return KM([
        ...
        [IK("⚙️ Config",    "cfg"),    IK("📊 Status",   "status")],
    ])
...
def kb_cfg():
    return KM([
        ...
        [IK("📝 Opt 1 Name","cfg:opt1name"), IK("📝 Opt 2 Name","cfg:opt2name"), IK("📝 Opt 3 Name","cfg:opt3name")],
...
async def cb_main(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    if d.startswith("cfg:") and d not in ("cfg:rm1","cfg:rm2"):
        ...
        prompts = {
            ...
            "opt1name":"Enter new name for Option 1:","opt2name":"Enter new name for Option 2:","opt3name":"Enter new name for Option 3:","opts":'Option Names: `"Opt1" "Opt2" "Opt3"`',
